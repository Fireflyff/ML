...
class QueryDataset(Dataset):
    ...
    def __init__(self, data, is_train=True, max_length=100):
...
class BertLinear(Module):
    ...
    def __init__(self, model_name, max_length=100, num_class=1):
...
class Trainer(object):
    ...
    def train(self, folds, epochs, train, check_number=5):
        ...
        for fold, (train_index, val_index) in enumerate(
                KFold(n_splits=folds, shuffle=True, random_state=37).split(train)):
            print(f'fold: {fold}')
            val_score_max = 0
...
submission.to_csv(f'submission_{model_use}.csv', index=False)
submission.head(20)